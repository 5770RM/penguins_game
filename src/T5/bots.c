...
int bot123_possible_placement(struct board_tile **board, struct player *players, struct bot_placement *placements, int x_size, int y_size)
{
    srand(time(0));
    int place_counter = 0;
...
int bot12_choosing_penguin(struct board_tile **board, struct player *players, struct bot_choosing *choice, int x_size, int y_size, int curr_player)
{
    srand(time(0));
...
void execute_placement_bot(struct board_tile **board, int x, int y, struct player *players, int n, int curr_player)
{
    ...
    case 1:
    {
        int placement_decision = bot123_possible_placement(board, players, placements, x, y);
        bot_placement_execution(board, players, placements, p, curr_player, placement_decision);
        break;
    }
    case 2:
    {
        int placement_decision = bot123_possible_placement(board, players, placements, x, y); // has the same way of choosing placement as 1st
        bot_placement_execution(board, players, placements, p, curr_player, placement_decision);
        break;
    }
...
void execute_movement_bot(struct board_tile **board, int x, int y, struct player *players, int n, int curr_player)
{
    ...
    case 1:
    {
        int penguin_decision = bot12_choosing_penguin(board, players, choice, x, y, curr_player);
    ...
    case 2:
    {
        int penguin_decision = bot12_choosing_penguin(board, players, choice, x, y, curr_player); // same choosing
        m = bot_choosing_execution(board, players, choice, m, curr_player, penguin_decision);
